...
- [ ] [debt] Exempt `context.DeadlineExceeded` from pipe-error warn in `proxyTarget` io.Copy guard (pre-existing, relocated by refactor) (source: pr-review-toolkit:review-pr) — internal/proxy/handler.go:114
- [ ] [debt] Log (with `redactURL`) when `http.NewRequestWithContext` fails before returning 500 (pre-existing) (source: pr-review-toolkit:review-pr) — internal/proxy/handler.go:73
- [ ] [debt] Early-return on raw `context.Canceled`/`DeadlineExceeded` from `fetchWithRetry` backoff instead of generic silent 502 (pre-existing) (source: pr-review-toolkit:review-pr) — internal/proxy/handler.go:82
